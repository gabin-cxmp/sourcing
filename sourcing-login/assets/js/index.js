import { requireAuth, sessionState, signOut, CONFIG } from './core.js';
...
const App = {
  ...
  async initLogin() {
    ...
    loginForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (error) {
          const friendly = error.message?.toLowerCase().includes('not found') ? 'User not found.' : (error.message || 'An error occurred.');
  ...
  disableEditMode() {
    const { DOM } = window.ui || { DOM: (s) => document.querySelector(s) };
    const controls = [
      '#supplierName', '#certifications', '#countrySelect',
      '#focusSelect', '#mainCategorySelect', '#secondaryCategorySelect'
    ].map(sel => DOM.qs(sel)).filter(Boolean);
  ...
  async saveSupplierData() {
    ...
    const payload = {
      name: DOM.qs('#supplierName')?.value?.trim() || null,
      company_certifications: DOM.qs('#certifications')?.value?.trim() || null,
      country: DOM.qs('#countrySelect')?.value || null,
      focus: DOM.qs('#focusSelect')?.value || null,
      main_product_category: DOM.qs('#mainCategorySelect')?.value || null,
      secondary_product_category: DOM.qs('#secondaryCategorySelect')?.value || null,
    };
    ...
    DOM.setLoading(editBtn, true, 'Save informations');
    ...
    try {
      ...
      DOM.setLoading(editBtn, false, 'Update');
    }
  },
  ...
  async populateSelects(formElements, product) {
    const { DOM } = window.ui || { DOM: { populateSelect: () => {} } };
    try {
      ...
      DOM.populateSelect(formElements.production_volumes, volumes, {
        valueKey: 'id', labelKey: 'quantity', selectedValue: product?.production_volumes
      });

      DOM.populateSelect(formElements.made_in, countries, {
        valueKey: 'id', labelKey: 'name', selectedValue: product?.made_in
      });

      DOM.populateSelect(formElements.recycled_organic, recycled, {
        valueKey: 'id', labelKey: 'name', selectedValue: product?.recycled_organic
      });

      DOM.populateSelect(formElements.raw_material_certifications, rawCerts, {
        valueKey: 'id', labelKey: 'name', selectedValue: product?.raw_material_certifications
      });
  ...
  async openDeleteModal(product) {
    ...
    confirmBtn.onclick = async () => {
      ...
      try {
        ...
        DOM.showMessage(DOM.qs('#message'), 'Delete failed.', 'error');
        modalClose();
      }
    };
  }
